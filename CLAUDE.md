...
- Admin templates stay **pure Django admin**: `templates/admin/base_site.html` must not include frontend CSS (`includes/styles.html` loads Bootstrap, whose global rules — `box-sizing: border-box`, `body { font-size }` — break admin button alignment). Site branding lives in `EAW/admin.py` (`site_header`/`site_title`/`index_title`); the built-in "View site" link (`site_url="/"` on every admin page) is the way back to the homepage.
- Input page duplicate-submit protection: one-time `submit_token` (session-remembered) server-side + button-disable client-side; batch insert is atomic (`bulk_create` in `transaction.atomic`); blank/colon-only lines are skipped.
