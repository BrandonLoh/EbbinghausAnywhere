...
class WeChatProfile(models.Model):
    """微信 openid 与 Web 账号的绑定关系(N:1)。

    多个微信可绑定同一个主站账号(家庭共用场景:如父母各自用微信
    帮孩子复习同一份数据);每个微信(openid)同时只属于一个账号,
    换绑时该微信会迁移到新账号。
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='wechat_profiles')
